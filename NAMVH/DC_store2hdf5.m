function [curr_dat_sz, curr_lab_sz] = DC_store2hdf5(filename, data, labels, create, startloc, chunksz, data_type, compress_level)  
  % *data* is W*H*C*N matrix of images
  ...
  if create
    ...
    h5create(filename, '/data', [dat_dims(1:end-1) Inf], 'Datatype', data_type, 'Deflate', compress_level, 'ChunkSize', [dat_dims(1:end-1) chunksz]); % width, height, channels, number 
    h5create(filename, '/label', [lab_dims(1:end-1) Inf], 'Datatype', data_type, 'Deflate', compress_level, 'ChunkSize', [lab_dims(1:end-1) chunksz]); % width, height, channels, number 
